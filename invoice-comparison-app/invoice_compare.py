...
/* 2. Class for the image container (to center the image) */
.bounce-image-container {
    text-align: center; /* Centers the inline-block image */
    margin: 20px auto; /* Adds space above/below */
}

/* 3. Class to apply the animation to the image */
.bounce-image {
    animation: bounce 2s infinite; /* Run the bounce animation, loop infinitely */
    display: inline-block; /* Allows centering via text-align: center on container */
    max-width: 300px; /* Control image size */
    height: auto;
    transform-origin: bottom; /* Ensures the bounce effect looks correct */
}
</style>
